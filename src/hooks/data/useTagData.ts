...
export function useTagData() {
    ...
    if (
      ...
      const timeoutId = setTimeout(async () => {
        await runMigrations(
          tagData,
          setTagData,
          (migrationName, processed, total) => {
            setMigrationProgress({ migrationName, processed, total });
          }
        );
      ...
      const migrationState = {
        ...
        migrations: { cleanupEmptyTracks: true, addTrackMetadata: true },
      };
  ...
  const getOrCreateTrackData = (trackUri: string) => {
    if (!tagData.tracks[trackUri]) {
      const now = Date.now();
      const metadata = getCurrentTrackMetadata();
      ...
      // Add metadata if available and URI matches current player
      if (metadata && Spicetify?.Player?.data?.item?.uri === trackUri) {
        newTrackData.name = metadata.name;
        newTrackData.artists = metadata.artists;
      }
    ...
    // If track exists but missing metadata, try to add it
    const existingTrack = tagData.tracks[trackUri];
      ...
      if (metadata && Spicetify?.Player?.data?.item?.uri === trackUri) {
        const updatedTagData = {
          ...
          tracks: {
            ...
            [trackUri]: {
              ...existingTrack,
              name: existingTrack.name || metadata.name,
              artists: existingTrack.artists || metadata.artists,
            },
          },
        };
